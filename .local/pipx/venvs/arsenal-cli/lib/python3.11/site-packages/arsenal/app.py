...
proxychains_str = 'PROXYCHAINS_CONF_FILE=$PWD/prox-.conf proxychains -q '
...
class App:
    ...
    def get_args(self):
        ...
        group_out.add_argument('-s', '--socks', action='store_true', help='prefix command with proxychains')
        ...
        while True:
            ...
            # OPT: Copy CMD to clipboard
                ...
                try:
                    import pyperclip
                    if args.socks:
                        pyperclip.copy(proxychains_str + cmd.cmdline)
                    else:
            ...
            # OPT: Only print CMD
                ...
                if args.socks:
                    print(proxychains_str + cmd.cmdline)
                else:
                    print(cmd.cmdline)
                break
            ...
            # OPT: Write in file
                ...
                with open(args.outfile, 'w') as f:
                    if args.socks:
                        f.write(proxychains_str + cmd.cmdline)
                    else:
                        f.write(cmd.cmdline)
                break
            ...
            # DEFAULT: Prefill Shell CMD
                ...
                self.prefil_shell_cmd(cmd, args.socks)
                break

    def prefil_shell_cmd(self, cmd, socks_state):
        print(cmd.cmdline)
        stdin = 0
        ...
        if socks_state:
            cmdline = proxychains_str + cmd.cmdline
        else: 
            cmdline = cmd.cmdline
        try:
            for c in cmdline:

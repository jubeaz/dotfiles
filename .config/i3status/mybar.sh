...
# bar {
#   status_command exec $XDG_CONFIG_HOME/i3status/mybar.sh
# }
...
mycrypto() {
  ...
  echo -n "\"full_text\":\" $($XDG_CONFIG_HOME/i3status/crypto.py) \","
  echo -n "\"color\":\"#000000\","
...
myip_public() {
  ...
  echo -n ",{"
  echo -n "\"name\":\"ip_public\","
  echo -n "\"full_text\":\" $($XDG_CONFIG_HOME/i3status/ip.py) \","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "},"
}
...
disk_usage() {
  ...
  echo -n "\"full_text\":\"  $($XDG_CONFIG_HOME/i3status/disk.py)%\","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "}"
}
...
memory() {
  ...
  echo -n "\"full_text\":\"  $($XDG_CONFIG_HOME/i3status/memory.py)%\","
...
cpu_usage() {
  ...
  echo -n "\"full_text\":\"  $($XDG_CONFIG_HOME/i3status/cpu.py)% \","
  echo -n "\"background\":\"#3949AB\","
  common
  echo -n "},"
}
...
meteo() {
  ...
  echo -n "\"full_text\":\" $($XDG_CONFIG_HOME/i3status/meteo.py) \","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "},"
}
...
while read line;
do
  # echo $line > $XDG_CONFIG_HOME/gitclones/github/i3/tmp.txt
  ...
  # CHECK UPDATES
    ...
    alacritty -e $XDG_CONFIG_HOME/i3status/click_checkupdates.sh &
...
#  elif [[ $line == *"name"*"id_crypto"* ]]; then
 #   xdg-open https://www.livecoinwatch.com/ > /dev/null &
  ...
  # LOGOUT
    ...
    i3-nagbar -t warning -m 'Log out ?' -b 'yes' 'pkill -9 -f lxsession' > /dev/null &
